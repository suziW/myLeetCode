from my_utils import matrixPrint
from typing import List
class Solution:
    def pacificAtlantic(self, matrix: List[List[int]]) -> List[List[int]]:  # none: 00, atlantic 10, pacific 01, both 11
        if matrix == []: return []
        raw = len(matrix)
        column = len(matrix[0])
        print(raw, column)
        ...
        def dfs(r, c, water):
            if record[r][c] in [water, 3]: return
            nonlocal count
            count += 1
            record[r][c] |= water
            for d in [(0, -1), (0, 1), (1, 0), (-1, 0)]:
                if r + d[0] < 0 or c + d[1] < 0 or r + d[0] > raw-1 or c + d[1] > column-1:
                    continue
                elif matrix[r+d[0]][c+d[1]] >= matrix[r][c]:
